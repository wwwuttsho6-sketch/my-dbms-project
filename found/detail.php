...
$stmt = $pdo->prepare("SELECT found_items.*, users.name as finder_name, users.department, users.email 
                       FROM found_items 
...
<div class="row my-4 gy-4">
    ...
    <div class="col-md-6">
        <div class="card card-custom h-100 p-4 d-flex flex-column">
            <div>
                <span class="badge badge-custom badge-found mb-3 align-self-start py-2 px-3">
            ...
            <table class="table table-custom my-3">
                <tr>
                    <td style="width: 35%;" class="text-muted"><i class="bi bi-geo-alt me-1 text-success"></i> Found Location:</td>
                    <td class="fw-semibold text-white"><?php echo htmlspecialchars($item['location']); ?></td>
                </tr>
                <tr>
                    <td class="text-muted"><i class="bi bi-calendar3 me-1 text-info"></i> Date Found:</td>
                    <td class="fw-semibold text-white"><?php echo htmlspecialchars($item['found_date']); ?></td>
                </tr>
                <tr>
                    <td class="text-muted"><i class="bi bi-person me-1 text-primary"></i> Found By:</td>
                    <td class="fw-semibold text-white"><?php echo htmlspecialchars($item['finder_name']); ?> (<?php echo htmlspecialchars($item['department']); ?>)</td>
                </tr>
                <tr>
                ...
                <?php if(isset($_SESSION['user_id'])): ?>
                    <?php if($_SESSION['user_id'] == $item['user_id']): ?>
                        <div class="alert alert-info bg-dark border-primary text-info text-center rounded-3">
                            <i class="bi bi-info-circle me-1"></i> You published this post. Review incoming claims in your dashboard.
                        </div>
                    <?php else: ?>
                        <a href="../dashboard/verification_form.php?type=found&item_id=<?php echo $item['id']; ?>" class="btn btn-brand btn-lg w-100 py-3 font-heading shadow">
                            <i class="bi bi-shield-lock me-2"></i> Claim This Belongs To Me (Fill Form)
                        </a>
                    <?php endif; ?>
                <?php else: ?>
                    <a href="../auth/login.php" class="btn btn-brand btn-lg w-100 py-3 font-heading">
                        <i class="bi bi-box-arrow-in-right me-2"></i> Sign In to Claim Item
                    </a>
                <?php endif; ?>
                
                <a href="index.php" class="btn btn-link w-100 text-center text-muted text-decoration-none mt-2 small">
                    &larr; Back to Found Feed
                </a>
            </div>
        </div>
    </div>
