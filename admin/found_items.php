...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'toggle_status') {
    $item_id = intval($_POST['item_id']);
    $new_status = $_POST['new_status'] === 'Returned' ? 'Returned' : 'Found';
...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h2 class="font-heading mb-1">🟢 Found Items Management</h2>
        <p class="text-muted mb-0">Review, moderate, or update status of all reported found items.</p>
    </div>
</div>
            ...
            <table class="table table-custom mb-0">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="small text-muted"><?php echo date('M d, Y', strtotime($item['found_date'])); ?></td>
                        <td>
                        ...
                        <td class="pe-4 text-end">
                            <div class="d-inline-flex gap-2">
                                <form action="found_items.php" method="POST">
                                    <input type="hidden" name="action" value="toggle_status">
                                    <input type="hidden" name="item_id" value="<?php echo $item['id']; ?>">
                                    <input type="hidden" name="new_status" value="<?php echo $item['status'] === 'Returned' ? 'Found' : 'Returned'; ?>">
                                    <button type="submit" class="btn btn-outline-custom btn-sm">
                                        Mark as <?php echo $item['status'] === 'Returned' ? 'Found' : 'Returned'; ?>
                                    </button>
                                </form>

                                <a href="<?php echo $base_url; ?>found/detail.php?id=<?php echo $item['id']; ?>" class="btn btn-secondary-custom btn-sm" target="_blank">
                                    <i class="bi bi-eye"></i>
                                </a>

                                <form action="found_items.php" method="POST" onsubmit="return confirm('Permanently delete this found item post?');">
                    ...
                    <?php endforeach; ?>
                    <?php if(empty($items)): ?>
                        <tr><td colspan="7" class="text-center py-4 text-muted">No found items posted yet.</td></tr>
                    <?php endif; ?>
                </tbody>
            </table>
        </div>
